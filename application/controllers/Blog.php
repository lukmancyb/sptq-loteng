...
class Blog extends CI_Controller
{
    ...
    function kategori()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $x['category'] = $this->db->get('tbl_kategori');
            $x['populer'] = $this->db->query("SELECT * FROM tbl_tulisan ORDER BY tulisan_views DESC LIMIT 5");
            $this->session->set_flashdata('kategori-msg', null);

            $this->load->view('depan/v_blog', $x);
        } else {
            $this->session->set_flashdata('kategori-msg', '<div class="alert alert-danger">Tidak Ada artikel untuk kategori <b>' . $kategori . '</b></div>');
            redirect('artikel');
        }
    }
